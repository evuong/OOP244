...
namespace aid {
    ...
    Good& Good::operator=(const Good& copy) {
        ...
        delete[] good_name;
        good_name = new char [strlen(copy.good_name)];
        name(copy.good_name);
    ...
    std::fstream& Good::store(std::fstream& file, bool newLine) const {
        file << good_type << ',' << good_sku << ',' << good_unit << ',' << good_name << ',' << on_hand << ',' << amount_needed << ',' << taxable << ',' << price_notax << ',' << state;
    ...
    std::fstream& Good::load(std::fstream& file) {
        ...
        if (file.is_open()) {
            ...
            *this = Good(sku2, name2, unit2, onhand2, taxable2, price_notax2, needed2);
        }
        return file;
    }
    ...
    std::istream& Good::read(std::istream& is) {
        char* newname = new char[max_name_length+1];
        char tax;
        is.clear();
    ...
    bool Good::operator>(const Good& copy) const {
    ...
    std::ostream& operator<<(std::ostream& os, const Good& current) {
        return current.write(os, true);
    }
    
    std::istream& operator>>(std::istream& is, Good& current) {
        current.read(is);
        return is;
    }
    
    double operator+=(double& add, const Good& current) {
